...
function loadMusic(indexNumb) {
    if (indexNumb >= 1 && indexNumb <= allMusic.length && allMusic[indexNumb - 1]) {
        musicName.innerText = allMusic[indexNumb - 1].name;
        musicArtist.innerText = allMusic[indexNumb - 1].artist;
        musicImg.src = allMusic[indexNumb - 1].img ? `images/${allMusic[indexNumb - 1].img}` : '';
        mainAudio.src = `songs/${allMusic[indexNumb - 1].src}.mp3`;
    } else {
        console.error(`Invalid index or undefined or undefined element: ${indexNumb}`);
    }
}
...
function nextMusic() {
    ...
    //if musicIndex is greater than array length then musicIndex will be 1 so the first music plays
...
function prevMusic() {
    musicIndex--; //increment of musicIndex by 1
    //if musicIndex is less than array length then musicIndex will be 1 so the first music plays
...
mainAudio.addEventListener("timeupdate", (e) => {
    ...
    mainAudio.addEventListener("loadeddata", () => {
        ...
        let loadedDataDuration = mainAudio.duration;
        let totalMin = Math.floor(loadedDataDuration / 60);
        let totalSec = Math.floor(loadedDataDuration % 60);
...
//above we just change icon, now we are going to work on what to do after song ended
...
for (let i = 0; i < allMusic.length; i++){
    ...
    <span id="${allMusic[i].src}" class="audio-duration">2:56</span>
    </li>`;
    ulTag.insertAdjacentHTML("beforeend", liTag);

    liAudioDurationTag = ulTag.querySelector(`#${allMusic[i].src}`);
    ...
    liAudioTag.addEventListener("loadeddata", () => {
        ...
        //adding t-duration with total duration value
...
function playingSong(){
    for(let j = 0; j < allLiTags.length; j++) {
        ...
        //let remove playing class from all other li expect the last one which is 
        if(allLiTags[j].classList.contains("playing")){
            allLiTags[j].classList.remove("playing");
        //audio duration value pass to .audio.duration inner text
